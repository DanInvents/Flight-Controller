...
void kalmanFilter(){
  for (int k=0; k<5; k++){
    ...
    else if (k < 4){ // Kalman filter parameters for rotation rate estimation
    ...
    else if (k == 4){ // Kalman filter parameters for acceleration estimation
    Ppe0[k] = P1[k] + Q[2]; // Update of prior estimation of "error covariance"
